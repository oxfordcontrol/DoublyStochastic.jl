...
mutable struct Data{T, Ti}
    ...
    function Data(C::SparseMatrixCSC{T, Ti}; rho=50.0, sigma=1.0, alpha=1.7, ε_abs=1e-4, ε_rel=0.0, print_interval=50) where {T, Ti}
...
function compute_factorization!(data::Data)
...
function print_info(data::Data)
    ...
    Ax = mul_A(data.C, data.x)
    r_prim = norm(Ax .- 1, Inf)
    r_dual = norm(data.H.*data.x - data.C.nzval + mul_At(data.C, data.y) + data.w, Inf)
...
function admm(C::SparseMatrixCSC; max_iterations=5000, kwargs...)
    data = Data(C; kwargs...)
    compute_factorization!(data)
...
function admm(C::SparseMatrixCSC, data::Data; max_iterations=5000)
    data.iteration = 0
...
function solve_linear_system!(data::Data)
    ...
    data.z .= mul_A(data.C, data.x_./(data.H .+ data.sigma))
...
function mul_A(C::SparseMatrixCSC, x)
    n = size(C, 1)
...
function mul_At(C::SparseMatrixCSC, x)
    n = size(C, 1)
    y = zeros(nnz(C))
    @inbounds for j in 1:n, idx in C.colptr[j]:C.colptr[j+1]-1
        i = C.rowval[idx]
